...
Generates a zero-dependency, 100% offline-ready single-file HTML application and compiles V-MAX_GUIDE.pdf.
"""
...
/* ========================================================= */
/* PRINT TO PDF STYLESHEET (PROFESSIONAL EXECUTIVE WHITEPAPER) */
/* ========================================================= */
@media print {
  @page {
    size: A4 portrait;
  ...
  .bg-grid, .action-bar, .nav-tabs, .btn, #btn-simulate, #toast, .copy-btn, .ide-selector {
  ...
  /* FORCE ALL TABS AND SECTIONS VISIBLE IN PRINT */
  .tab-content, .ide-content {
  ...
  .badge-tag span.pulse { background: #059669 !important; box-shadow: none !important; }
  ...
  .comp-box.without { border: 1px solid #fca5a5 !important; background: #fff5f5 !important; }
  .comp-box.with { border: 1px solid #86efac !important; background: #f0fdf4 !important; }
  .comp-box.without h3 { color: #b91c1c !important; }
  .comp-box.with h3 { color: #15803d !important; }
  .comp-list li { color: #374151 !important; font-size: 9.5pt !important; }
  ...
  .doc-num { color: #0284c7 !important; font-size: 8.5pt !important; }
  .doc-title { color: #111827 !important; font-size: 11pt !important; }
  .doc-path { background: #e0e7ff !important; color: #3730a3 !important; font-size: 8.5pt !important; border: 1px solid #c7d2fe !important; }
  .doc-body { color: #4b5563 !important; font-size: 9pt !important; }
  ...
  .agent-name { color: #111827 !important; font-size: 9pt !important; }
  .agent-role { color: #4b5563 !important; font-size: 7.5pt !important; }
...
<div class="container">
  ...
  <header>
    <div class="badge-tag">
      ...
      <span>VIBEMAX FRAMEWORK v1.0 • OPEN SOURCE SPECIFICATION</span>
    </div>
    ...
    <div class="print-meta-bar">
      <span><strong>Official GitHub:</strong> https://github.com/vickee2025/v-max</span>
      <span><strong>Author:</strong> Vickee (Tecvic)</span>
    </div>

    <div class="action-bar">
      <a href="https://github.com/vickee2025/v-max" target="_blank" class="btn btn-primary">
    ...
    <div class="card">
      ...
      <div class="comparison-grid">
        ...
        <div class="comp-box with">
          <h3><span>⚡</span> V-Max Living Docs Solution</h3>
          <ul class="comp-list">
    ...
    <div class="card">
      ...
      <div class="swarm-container">
        <div class="swarm-header">
          <div>
            <strong style="font-size:14px;">Swarm Execution Topology</strong>
            <div style="font-size:11px; color:var(--text-muted);">Parallel dispatch across 5 inspection vectors</div>
          </div>
          <button id="btn-simulate" onclick="simulateSwarm()" class="btn btn-primary" style="padding:6px 14px; font-size:12px;">
  ...
  <!-- SECTION 4: VISION HANDSHAKE -->
  <section id="tab-handshake" class="tab-content">
    <div class="card">
      ...
      <div style="border-radius:10px; padding:16px; margin-bottom:12px; border: 1px solid var(--border);">
        <h3 style="font-size:15px; margin-bottom:10px;">The 4-Step Handshake Protocol:</h3>
  ...
  <section id="tab-setup" class="tab-content">
    <div class="card">
      ...
      <div id="ide-antigravity" class="ide-content">
        <h4 style="font-size:13px; margin-bottom:6px;">Google Antigravity & Gemini CLI (Global 1-Click /vibemax):</h4>
        <div class="code-box">
      ...
      <div id="ide-claude" class="ide-content">
        <h4 style="font-size:13px; margin-bottom:6px;">Anthropic Claude Code:</h4>
        <div class="code-box">
          <button class="copy-btn" onclick="copyCode('prompt-claude')">Copy Prompt</button>
          <pre id="prompt-claude">Please read vibemax/SKILL.md and execute the Vibemax workflow: scan this project, create the 6 living docs in docs/, conduct the vision handshake, and update CLAUDE.md.</pre>
        </div>
      </div>

      <div id="ide-cursor" class="ide-content">
        <h4 style="font-size:13px; margin-bottom:6px;">Cursor IDE (Composer: Cmd+I / Ctrl+I):</h4>
        <div class="code-box">
          <button class="copy-btn" onclick="copyCode('prompt-cursor')">Copy Prompt</button>
          <pre id="prompt-cursor">@vibemax/SKILL.md Vibemax this project. Scan the codebase, create docs/ with all 6 essential documents, conduct the vision handshake, and update .cursorrules.</pre>
        </div>
      </div>

      <div id="ide-sync" class="ide-content">
        <h4 style="font-size:13px; margin-bottom:6px;">Daily Continuous Sync Prompt (Any Tool):</h4>
        <div class="code-box">
  ...
  <footer>
    ...
    <p style="margin-top:4px;">Official Repository: <a href="https://github.com/vickee2025/v-max" style="color:var(--accent-cyan); text-decoration:none;">https://github.com/vickee2025/v-max</a></p>
  </footer>
...
<script>
// Interactive Screen Tab Switcher (Screen only)
...
// Initialize on screen load (show tab 1, hide others)
window.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.tab-content').forEach((t, i) => {
    if (i > 0) t.style.display = 'none';
  });
  document.querySelectorAll('.ide-content').forEach((c, i) => {
...
def main():
    ...
    target_pdf = os.path.join(target_dir, "V-MAX_GUIDE.pdf")
    ...
    # Headless Chrome PDF compilation
    chrome_path = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
    if os.path.exists(chrome_path):
        print("📄 Compiling professional PDF via Headless Chrome...")
        cmd = [
            chrome_path,
            "--headless",
            "--disable-gpu",
        ...
        if res.returncode == 0:
            print(f"🎉 Generated pixel-perfect PDF at: {target_pdf}")
        else:
            print(f"⚠️ Chrome PDF warning: {res.stderr}")
